...
public class IndexedBlockStore extends AbstractBlockstore{
    ...
    //VIC: very important, we get the best block from the DB
    //VIC: we need to change it so if two chains are equally long,
    //VIC: then we select the chain with less Social Network users repeated
    //VIC: and under no circumstances can be the same Social Network user in 
    //VIC: two consecutive nodes
